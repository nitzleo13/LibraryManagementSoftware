...
<body role="document">
	...
	<c:choose>
			<c:when test="${payinfullResults == 'paidInFull' }">
								...
								<tr>
									<c:forEach var="borrower" items="${lookUpFineResults}"
										...
										<tr>
											...
											<td>
											<c:if test="${borrower.paid != 0}">
											<button type="button" class="btn btn-xs btn-warning" onclick="window.location.href='/bodhi/admin/fines/payfull?card_no=${borrower.card_no}'">Pay</button>
											</c:if>
											</td>
										</tr>
									</c:forEach>
